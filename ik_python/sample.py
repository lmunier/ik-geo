...
def run_ik_general():
    ...
    robot = ik_python.Robot("sphericalTwoParallel")
    ...
    ey = [0.0,1.0,0.0]
    ez = [0.0,0.0,1.0]
    hMat = np.array([ez,ey,ey,ex,ey,ex]).transpose()
    pMat = np.array([zv, [.32,0,.78], [0,0,1.075], [1.1425,0, 0.2], ez, ez, [0.2,0,0]]).transpose()
